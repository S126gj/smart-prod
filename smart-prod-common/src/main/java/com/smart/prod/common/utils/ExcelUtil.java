...
public class ExcelUtil {
     ...
     * @throws IOException
     */
    public static void downloadFailedUsingJson(String fileName, List<?> data, Class<?> object, HttpServletResponse response) throws IOException {
        try {
    ...
    public static <T> List<T> read (String path, String fileName, Class<T> object) {
        ...
        EasyExcel.read(String.format("%s/%s", path , fileName), object, new PageReadListener<T>(dataList -> list.addAll(dataList))).sheet().doRead();
        return list;
    }
    ...
    public static  <T> List<T> read (InputStream inputStream, Class<T> object) {
        ...
        // 具体需要返回多少行可以在`PageReadListener`的构造函数设置
        List<T> list = new ArrayList<>();
        EasyExcel.read(inputStream, object, new PageReadListener<T>(dataList -> list.addAll(dataList))).sheet().doRead();
        return list;
    }
}