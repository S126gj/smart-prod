...
public class RedisUtils {
     ...
     * @param keys
     */
    public void del(Set<Object> keys) {
    ...
    public Long getIncr(String key, long liveTime) {
        ...
        if (increment == 0) {
            increment = getIncr(key, liveTime);// 从1开始
        }
        return increment;
    }
    ...
    public Object lGetLast(String key) {
        try {
            if (hasKey(key)) {
                long size = lGetListSize(key);
                List<?> last = redisTemplate.opsForList().range(key, size - 1, size);
                if (last != null) {
                    return last.get(0);
                }
            }
     ...
     * @param index
     */
    public void lRemove(String key, long index) {
        try {
            redisTemplate.opsForList().set(key, index, "del");
            redisTemplate.opsForList().remove(key, 0, "del");
        } catch (Exception e) {
            log.error(e.getMessage(), e);
        }
     ...
     * @param keys
     */
    public void delByKeys(Set<Object> keys) {
        long count = redisTemplate.delete(keys);
        // 此处提示可自行删除
        log.debug("--------------------------------------------");
        log.debug("成功删除缓存：" + keys.toString());
