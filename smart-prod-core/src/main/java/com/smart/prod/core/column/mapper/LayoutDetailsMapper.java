package com.smart.prod.core.column.mapper;
...
public interface LayoutDetailsMapper extends BaseMapper<LayoutDetails> {
     ...
     */
    @Insert(
        "insert into sys_layout_d (id,layout_id,sort,field_name,label,width,input_type,visible,readonly,layout_type,data_source,validate_rules,filterable,auth_config,sortable )"
            +
            "values(REPLACE(RAND(),'0.',''),#{layoutId},#{sort},#{fieldName},#{label},#{width},#{inputType},#{visible},#{readonly},#{layoutType},#{dataSource},#{validateRules},#{filterable},#{authConfig},#{sortable})")
    void insert(
